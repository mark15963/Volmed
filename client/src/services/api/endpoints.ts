...
/* =======================
  ...
  updateGeneralConfig: (data: any) => requestWrapper("put", `/general/config`, data),
  getLogo: () => requestWrapper("get", `/general/get-logo`),
  uploadLogo: (formData: FormData) => requestWrapper("post", `/general/upload-logo`, formData),
  ...
  /* =================
     💊 MEDICATION API
     ================= */
  deleteMedication: (medId: string) => requestWrapper("delete", `/medications/${medId}`),
  getMedications: (patientId: string) => requestWrapper("get", `/patients/${patientId}/medications`),
  createMedication: (patientId: string, data: any) =>
    requestWrapper("post", `/patients/${patientId}/medications`, data),
  updateMedication: (medId: string, data: any) => requestWrapper("put", `/medications/${medId}`, data),

  /* ===================
     📂 PATIENT FILES API
     =================== */
  getPatientFiles: (patientId: string) => requestWrapper("get", `/patients/${patientId}/files`),
  uploadFile: (patientId: string, file: File) => {
    const formData = new FormData();
    formData.append("file", file);
    return requestWrapper("post", `/patients/${patientId}/upload`, formData);
  },
  deleteFile: (filePath: string) => requestWrapper("delete", `/files`, { data: { filePath } }),
  ...
  getPulseData: (patientId: string) => requestWrapper("get", `/patients/${patientId}/pulse`),
  saveO2: (patientId: string, o2Value: string) =>
    requestWrapper("post", `/patients/${patientId}/o2`, { o2Value }),
  getO2Data: (patientId: string) => requestWrapper("get", `/patients/${patientId}/o2`),
  ...
  /* ==========
     💬 CHAT API
     ========== */
  getChatHistory: (room: string) => requestWrapper("get", `/chat/room/${room}/messages`),
  saveMessage: (data: any) => requestWrapper("post", `/chat/save-message`, data),
  getActiveRooms: () => requestWrapper("get", `/chat/active-rooms`),
  deleteChatRoom: (room: string) => requestWrapper("delete", `chat/room/${room}`),
  ...
  updateUser: (id: string, data: any) => requestWrapper("put", `/users/${id}`, data),
